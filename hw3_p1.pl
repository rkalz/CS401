...
check(Input) :-
    ...
    (is_property(NameString, _) -> true; write("Unknown\n"), run), % Checks if name in dictionary
    (is_property(NameString, FixedAttr) -> write("Yes\n"); write("No\n")), run.
...
add(Input) :-
    ...
    remove_char(Attribute, '.', AtomAttr), atom_string(AtomAttr, FixedAttr), atom_string(Name, NameString), assert(property(NameString, FixedAttr)), write("OK"), nl, run.
